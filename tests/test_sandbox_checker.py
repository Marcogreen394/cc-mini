...
from core.sandbox.checker import DependencyCheck, check_dependencies
...
class TestCheckDependencies:
    def test_non_linux_platform(self, monkeypatch):
        monkeypatch.setattr("core.sandbox.checker.platform.system", lambda: "Darwin")
    ...
    def test_userns_disabled(self, monkeypatch, tmp_path):
        monkeypatch.setattr("core.sandbox.checker.platform.system", lambda: "Linux")
        monkeypatch.setattr("core.sandbox.checker.shutil.which", lambda x: "/usr/bin/bwrap")
        userns_file = tmp_path / "unprivileged_userns_clone"
        userns_file.write_text("0")
        monkeypatch.setattr(
            "core.sandbox.checker.Path",
    ...
    def test_bwrap_test_failure(self, monkeypatch):
        ...
        monkeypatch.setattr("core.sandbox.checker.Path", lambda p: FakePath(p))

        mock_proc = MagicMock()
        mock_proc.returncode = 1
        mock_proc.stderr = b"some error"
        monkeypatch.setattr("core.sandbox.checker.subprocess.run", lambda *a, **kw: mock_proc)
    ...
    def test_bwrap_test_timeout(self, monkeypatch):
        ...
        monkeypatch.setattr("core.sandbox.checker.subprocess.run", raise_timeout)
